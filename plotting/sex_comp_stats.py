...
mask_day_1_to_3 = (df['Day_number'] >= 1) & (df['Day_number'] <= 3)
mask_day_4_to_9 = (df['Day_number'] >= 4) & (df['Day_number'] <= 9)
mask_day_10_to_22 = (df['Day_number'] >= 10) & (df['Day_number'] <= 22)
mask_day_23_to_27 = (df['Day_number'] >= 23) & (df['Day_number'] <= 27)

# Create new DataFrames for each day range with needed columns
panic_df = df.loc[mask_day_1_to_3, ['Sex', 'Tank_number', 'ID', 'Freezing_fraction', 'frantic_fraction']]
stress_df = df.loc[mask_day_4_to_9, ['Sex', 'Tank_number', 'ID', 'Freezing_fraction', 'frantic_fraction']]
hab_df = df.loc[mask_day_10_to_22, ['Sex', 'Tank_number', 'ID', 'Freezing_fraction', 'frantic_fraction']]
hab_df = df.loc[mask_day_23_to_27, ['Sex', 'Tank_number', 'ID', 'Freezing_fraction', 'frantic_fraction']]
...
# Perform t-test for normally distributed data, and Mann-Whitney U test for non-normally distributed data
if p_value_freezing_male > 0.05 and p_value_freezing_female > 0.05:
    ...
    u_statistic_freezing, p_value_freezing = mannwhitneyu(data_male['Freezing_fraction'], data_female['Freezing_fraction'])
...
if p_value_frantic_male > 0.05 and p_value_frantic_female > 0.05:
    ...
    u_statistic_frantic, p_value_frantic = mannwhitneyu(data_male['frantic_fraction'], data_female['frantic_fraction'])
...
if 't_statistic_freezing' in locals():
...
if 't_statistic_frantic' in locals():
